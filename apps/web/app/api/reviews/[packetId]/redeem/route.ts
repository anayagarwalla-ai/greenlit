...
export async function POST(request: Request, context: { params: Promise<{ packetId: string }> }) {
  ...
  try {
    ...
    const { data: packet, error } = await database.from("review_packets_v2").select("id, record_id, snapshot, snapshot_sha256, expires_at, revoked_at, decision, reviewer_name, reviewer_email, reviewer_note, decided_at, receipt_sha256, decision_event_hash").eq("public_id", packetId).eq("bearer_token_hash", sha256(parsed.data.token)).single();
