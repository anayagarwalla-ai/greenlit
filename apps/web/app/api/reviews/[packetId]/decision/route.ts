...
export async function POST(request: Request, context: { params: Promise<{ packetId: string }> }) {
  ...
  try {
    ...
    const { data: packet, error } = await database.from("review_packets_v2").select("id, record_id, snapshot, snapshot_sha256, expires_at, decision").eq("public_id", packetId).eq("session_hash", sha256(session)).single();
