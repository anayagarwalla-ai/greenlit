...
import { hydrateReviewEvidence, reviewSessionAuthorized, reviewSessionCookieName } from "@/lib/review-session";
...
export async function GET(_request: Request, context: { params: Promise<{ packetId: string }> }) {
  ...
  try {
    ...
    const { data: packet, error } = await database.from("review_packets_v2").select("id, record_id, snapshot, snapshot_sha256, expires_at, revoked_at, decision, reviewer_name, reviewer_email, reviewer_note, decided_at, receipt_sha256").eq("public_id", packetId).single();
