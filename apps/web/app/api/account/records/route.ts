...
export async function GET() {
  ...
  try {
    ...
    const [runResult, reviewResult, notificationResult, invoiceResult, invoiceJobResult, invoicePlanResult, stripeResult] = await Promise.all([
    ...
    const queryError = runResult.error ?? reviewResult.error ?? notificationResult.error ?? invoiceResult.error ?? invoiceJobResult.error ?? invoicePlanResult.error ?? stripeResult.error;
    ...
    return NextResponse.json({
      ...
      records: (records ?? []).map((record) => ({ ...record, latestRun: runMap[record.id] ?? null, latestReview: reviewMap[record.id] ?? null, latestInvoice: invoiceMap[record.id] ?? null, latestInvoiceJob: invoiceJobMap[record.id] ?? null, invoicePlan: invoicePlanMap[record.id] ?? null })),
