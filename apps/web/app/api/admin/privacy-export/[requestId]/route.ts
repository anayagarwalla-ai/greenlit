...
export async function GET(_request: Request, context: { params: Promise<{ requestId: string }> }) {
  ...
  try {
    ...
    const subjectEmail = privacyRequest.email.trim().toLowerCase();
    let ownerId = "";
    for (let page = 1; page <= 5 && !ownerId; page += 1) {
      const { data, error } = await database.auth.admin.listUsers({ page, perPage: 200 });
      if (error) throw new Error(error.message);
      ownerId = data.users.find((account) => account.email?.toLowerCase() === subjectEmail)?.id ?? "";
      if (data.users.length < 200) break;
    }

    // Whole records and their operational/audit data belong only in an
    // account-owner export. A reviewer or billing contact receives only the
    // fields that identify and describe that person's own participation.
    const { data: ownerRecords, error: ownerRecordsError } = ownerId
      ? await database.from("transaction_records").select("id,public_id,owner_user_id,mode,agency_name,client_name,project_name,milestone_title,amount_minor,currency,source_name,source_sha256,confirmed_criteria,target_origin,revision,criteria_revision,status,retention_until,legal_hold,created_at,updated_at,privacy_deletion_requested_at,deletion_status").eq("owner_user_id", ownerId).order("created_at")
      : { data: [], error: null };
    ...
    const [runs, ownerReviews, events, amendments, notifications, feedbackByOwner, feedbackByEmail, relatedPrivacyRequests, consentEvents, invite, holds, productEvents, accountDeletions, stripeConnection, stripeConnectionEvents, ownerInvoicePlans, ownerInvoiceJobs, ownerInvoices, reviewerPackets, billingPlans, billingInvoices] = await Promise.all([
      ...
      ownerRecordIds.length ? database.from("privacy_record_amendments").select("id,request_id,record_id,field_name,previous_value,corrected_value,reason,created_at").in("record_id", ownerRecordIds).order("created_at") : Promise.resolve({ data: [], error: null }),
      ...
      database.from("beta_feedback").select("id,public_id,record_id,email,category,message,page_path,status,created_at").ilike("email", subjectEmail).order("created_at"),
      database.from("privacy_requests_v2").select("id,public_id,request_type,email,details,status,assigned_to,identity_verified_at,response_summary,response_sent_at,created_at,completed_at").ilike("email", subjectEmail).order("created_at"),
      ...
      database.from("beta_invites").select("id,email,status,adult_sponsor,invited_by,invited_at,last_sign_in_requested_at,removed_at,notes").ilike("email", subjectEmail).maybeSingle(),
      ...
      database.from("privacy_account_deletions").select("id,request_id,email,status,attempts,last_error,requested_at,completed_at").ilike("email", subjectEmail).order("requested_at"),
      ...
      ownerRecordIds.length ? database.from("invoice_jobs").select("id,packet_id,record_id,status,attempts,plan,claimed_at,completed_at,last_error,created_at,updated_at").in("record_id", ownerRecordIds).order("created_at") : Promise.resolve({ data: [], error: null }),
      ownerRecordIds.length ? database.from("record_invoices").select("id,packet_id,record_id,stripe_account_id,stripe_customer_id,stripe_invoice_id,invoice_number,status,amount_due_minor,amount_paid_minor,currency,billing_email,due_at,hosted_invoice_url,invoice_pdf_url,sent_at,paid_at,voided_at,last_error,last_stripe_event_id,last_stripe_event_at,created_at,updated_at").in("record_id", ownerRecordIds).order("created_at") : Promise.resolve({ data: [], error: null }),
      database.from("review_packets_v2").select("record_id,public_id,decision,reviewer_name,reviewer_email,reviewer_note,intent_confirmed,legal_terms_accepted,electronic_records_consent,notice_version,country_code,decided_at,receipt_sha256,created_at").ilike("reviewer_email", subjectEmail).order("created_at"),
      database.from("record_invoice_plans").select("record_id,billing_name,billing_email,days_until_due,memo,amount_minor,currency,criteria_revision,created_at,updated_at").ilike("billing_email", subjectEmail).order("created_at"),
      database.from("record_invoices").select("record_id,packet_id,invoice_number,status,amount_due_minor,amount_paid_minor,currency,billing_email,due_at,hosted_invoice_url,invoice_pdf_url,sent_at,paid_at,voided_at,created_at,updated_at").ilike("billing_email", subjectEmail).order("created_at"),
    ]);
    ...
    const contextualRecordIds = [...new Set([...(reviewerPackets.data ?? []).map((item) => item.record_id), ...(billingPlans.data ?? []).map((item) => item.record_id), ...(billingInvoices.data ?? []).map((item) => item.record_id)])].filter((id) => !ownerRecordIds.includes(id));
    const { data: contexts, error: contextError } = contextualRecordIds.length ? await database.from("transaction_records").select("id,public_id").in("id", contextualRecordIds) : { data: [], error: null };
    if (contextError) throw new Error(contextError.message);
    const publicIdByRecord = new Map((contexts ?? []).map((item) => [item.id, item.public_id]));
    const minimize = <T extends { record_id: string }>(items: T[]) => items.filter((item) => !ownerRecordIds.includes(item.record_id)).map(({ record_id, ...item }) => ({ recordPublicId: publicIdByRecord.get(record_id) ?? null, ...item }));
