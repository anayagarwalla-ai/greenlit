...
export async function GET(_request: Request, context: { params: Promise<{ requestId: string }> }) {
  ...
  try {
    ...
    const [runs, reviews, events, amendments, notifications, feedbackByOwner, feedbackByEmail, relatedPrivacyRequests, consentEvents, invite, holds, productEvents, accountDeletions] = await Promise.all([
    ...
    const queryError = runs.error ?? reviews.error ?? events.error ?? amendments.error ?? notifications.error ?? feedbackByOwner.error ?? feedbackByEmail.error ?? relatedPrivacyRequests.error ?? consentEvents.error ?? invite.error ?? holds.error ?? productEvents.error ?? accountDeletions.error;
    ...
    const payload = { exportVersion: 2, exportedAt, privacyRequest, account: { email: privacyRequest.email, userId: ownerId || null }, invite: invite.data ?? null, accountDeletions: accountDeletions.data ?? [], records: records ?? [], verificationJobs: runs.data ?? [], reviewPackets: reviews.data ?? [], auditEvents: events.data ?? [], corrections: amendments.data ?? [], notifications: notifications.data ?? [], feedback, relatedPrivacyRequests: relatedPrivacyRequests.data ?? [], analysisConsents: consentEvents.data ?? [], legalHolds: holds.data ?? [], productEvents: productEvents.data ?? [] };
