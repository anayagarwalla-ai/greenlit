...
export async function GET(_request: Request, context: { params: Promise<{ requestId: string }> }) {
  ...
  try {
    ...
    const [runs, reviews, events, amendments, notifications, feedbackByOwner, feedbackByEmail, relatedPrivacyRequests, consentEvents, invite, holds, productEvents, accountDeletions, stripeConnection, invoicePlans, invoiceJobs, invoices] = await Promise.all([
      ...
      recordIds.length ? database.from("record_invoices").select("id,packet_id,record_id,stripe_account_id,stripe_customer_id,stripe_invoice_id,invoice_number,status,amount_due_minor,amount_paid_minor,currency,billing_email,due_at,hosted_invoice_url,invoice_pdf_url,sent_at,paid_at,voided_at,last_error,created_at,updated_at").in("record_id", recordIds).order("created_at") : Promise.resolve({ data: [], error: null }),
    ]);
    const queryError = runs.error ?? reviews.error ?? events.error ?? amendments.error ?? notifications.error ?? feedbackByOwner.error ?? feedbackByEmail.error ?? relatedPrivacyRequests.error ?? consentEvents.error ?? invite.error ?? holds.error ?? productEvents.error ?? accountDeletions.error ?? stripeConnection.error ?? invoicePlans.error ?? invoiceJobs.error ?? invoices.error;
