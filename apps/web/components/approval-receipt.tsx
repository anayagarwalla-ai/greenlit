  ...
  return (
    <main className="receipt-shell">
      ...
      <article className="receipt-page" aria-label="Milestone approval record">
        ...
        <section className="hash-block"><div><span className="receipt-section-title">{demo ? "Illustrative outcomes" : "Evidence snapshot"}</span><p>{snapshot.run.runId} · {snapshot.run.buildLabel}<br />{snapshot.run.browserVersion} · {snapshot.run.results.length} results<br />{demo ? "No manifest was generated" : `Manifest SHA-256: ${snapshot.run.manifestSha256}`}</p></div><div><span className="receipt-section-title">{demo ? "Walkthrough status" : "Canonical record"}</span><p>{snapshot.recordPublicId} · revision {snapshot.revision}<br />{demo ? "No snapshot, receipt, or audit hashes were generated" : <>Snapshot SHA-256: {packet.snapshotSha256}<br />Receipt SHA-256: {packet.receiptSha256}<br />Audit event {packet.auditHead?.sequence ?? "—"}: {packet.auditHead?.eventHash ?? "Unavailable"}</>}</p></div></section>
        ...
        <footer className="receipt-page__foot"><span>Generated by MilestoneProof</span><span>{demo ? "DEMO · NOT RETAINED" : packet.packetId}</span><span>Page 1 of 1</span></footer>
      </article>
