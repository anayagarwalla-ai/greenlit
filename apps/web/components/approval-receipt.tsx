    ...
    <main className="receipt-shell">
      ...
      <article className="receipt-page" aria-label="Milestone approval record">
        ...
        <ol className="stepper" aria-label="Milestone progress">
          {["Confirm criteria", "Verify build", "Client review", "Invoice-ready"].map((label, index) => <li className={index === 3 ? "step is-active" : "step is-done"} aria-current={index === 3 ? "step" : undefined} key={label}>{label}</li>)}
        </ol>
