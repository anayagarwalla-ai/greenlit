...
import { ArrowRight, Bell, Check, Clipboard, Clock3, ExternalLink, FileCheck2, LoaderCircle, LogOut, RefreshCw, ShieldCheck } from "lucide-react";
...
type Review = { public_id: string; decision?: "APPROVED" | "CHANGES_REQUESTED" | null; reviewer_name?: string | null; reviewer_email?: string | null; decided_at?: string | null; expires_at: string; revoked_at?: string | null; created_at: string };
...
export function AgencyDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    if (!response.ok) { setError(payload.error ?? "The dashboard could not be loaded."); return; }
    setData(payload);
  }, []);
  ...
  const claimCurrent = async () => {
    setBusy("claim");
    await fetch("/api/account/claim-current", { method: "POST" });
    await load();
    setBusy("");
  };
  ...
  const createReview = async (record: RecordItem) => {
    ...
    if (!response.ok) setError(payload.error ?? "A new review link could not be created.");
    else {
      setReviewLinks((current) => ({ ...current, [record.id]: payload.reviewUrl }));
      try { await navigator.clipboard.writeText(payload.reviewUrl); setCopied(record.id); } catch { setCopied(""); }
      await load();
    }
    setBusy("");
  };
  ...
  const reviewAction = async (record: RecordItem, action: "revoke" | "extend") => {
    ...
    const payload = await response.json();
    if (!response.ok) setError(payload.error ?? "The review could not be updated.");
    await load();
    setBusy("");
  };
  ...
  return (
    <main className="dashboard-shell">
      ...
      <div className="dashboard-main">
        <section className="dashboard-heading"><div><div className="legal-kicker">Closed agency beta</div><h1>Your milestone proofs</h1><p>Resume projects, watch client decisions, and keep approval records accessible across devices.</p></div><div className="dashboard-heading__actions"><button className="button button--outline" onClick={() => void claimCurrent()} disabled={busy === "claim"}>{busy === "claim" ? <LoaderCircle className="spin" size={15} /> : <RefreshCw size={15} />}Recover current browser project</button><Link className="button button--lime" href="/workspace">New milestone <ArrowRight size={15} /></Link></div></section>
