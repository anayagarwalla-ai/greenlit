...
function MappingFields({ criterion, draft, update }: { criterion: AnalysisCriterion; draft: CheckDraft; update: (patch: Partial<CheckDraft>) => void }) {
  return (
    <div className="mapping-fields">
      <label>Page path<input value={draft.path} onChange={(event) => update({ path: event.target.value })} placeholder="/contact" /></label>
      {(criterion.checkType === "element_state" || criterion.checkType === "link_destination") && <label>Element reference<input value={draft.elementRef} onChange={(event) => update({ elementRef: event.target.value })} placeholder="button:Get started" /><small>Exact accessible role:name, such as link:Contact us.</small></label>}
      {criterion.checkType === "element_state" && <><label>Assertion<select value={draft.assertion} onChange={(event) => update({ assertion: event.target.value as CheckDraft["assertion"] })}><option value="visible">Visible</option><option value="enabled">Enabled</option><option value="count">Exact count</option></select></label>{draft.assertion === "count" && <label>Expected count<input type="number" min="0" max="100" value={draft.expectedCount} onChange={(event) => update({ expectedCount: event.target.value })} /></label>}</>}
      {criterion.checkType === "link_destination" && <label>Expected same-origin path<input value={draft.expectedPath} onChange={(event) => update({ expectedPath: event.target.value })} placeholder="/contact" /></label>}
      {criterion.checkType === "form_submission" && <>
        <label className="mapping-wide">Test values<textarea value={draft.fields} onChange={(event) => update({ fields: event.target.value })} placeholder={"Email=qa@example.com\nMessage=MilestoneProof beta test"} /><small>One accessible field label and test value per line.</small></label>
        <label>Submit element<input value={draft.submitRef} onChange={(event) => update({ submitRef: event.target.value })} placeholder="button:Send" /></label>
        <label>Success message<input value={draft.successText} onChange={(event) => update({ successText: event.target.value })} placeholder="Thanks, we received it" /></label>
        <label>Success page path<input value={draft.successPath} onChange={(event) => update({ successPath: event.target.value })} placeholder="/thank-you (optional)" /></label>
        <label>Expected POST path<input value={draft.expectedPostPath} onChange={(event) => update({ expectedPostPath: event.target.value })} placeholder="/api/leads (optional)" /></label>
        <label>Expected status<input type="number" min="200" max="399" value={draft.expectedStatus} disabled={!draft.expectedPostPath.trim()} onChange={(event) => update({ expectedStatus: event.target.value })} /></label>
      ...
      {criterion.checkType === "viewport_layout" && <><label>Viewports<select value={draft.viewports} onChange={(event) => update({ viewports: event.target.value as CheckDraft["viewports"] })}><option value="both">Mobile + desktop</option><option value="mobile">Mobile only</option><option value="desktop">Desktop only</option></select></label><label>Maximum horizontal overflow (px)<input type="number" min="0" max="20" value={draft.maxOverflow} onChange={(event) => update({ maxOverflow: event.target.value })} /></label></>}
      {criterion.checkType === "axe_scan" && <><label>Optional submit element<input value={draft.submitRef} onChange={(event) => update({ submitRef: event.target.value })} placeholder="Leave blank for a page scan" /><small>Use only to expose validation messages before scanning.</small></label>{draft.submitRef.trim() && <label className="mapping-consent mapping-wide"><input type="checkbox" checked={draft.mutationAcknowledged} onChange={(event) => update({ mutationAcknowledged: event.target.checked })} /><span>I authorize this staging interaction.</span></label>}</>}
    </div>
  );
}
