    ...
    sections: [
      ...
      },
      {
        ...
        table: {
          ...
          rows: [
            ["0:00 to 0:12", "Failed launch-rc1 result", "This page says the contact request was sent. The API says it failed. A screenshot would call this done. Greenlit catches what actually happened."],
            ["0:12 to 0:28", "Homepage and walkthrough entry", "The agency says the milestone is finished. The client asks for proof. Greenlit follows each SOW promise from source language to an observed result and a clear decision."],
            ["0:28 to 0:48", "Synthetic SOW and AC-04", "Greenlit drafts measurable criteria with exact source quotes, and the agency confirms every requirement before it is frozen."],
            ["0:48 to 1:08", "Criteria and typed checks", "Supported promises map to allowlisted checks, not arbitrary generated code. The visible success and HTTP 500 are preserved together against the same criterion."],
            ["1:08 to 1:25", "Passing launch-rc2 result", "The agency fixes the build and reruns the same frozen criteria. There is no moving the goalposts."],
            ["1:25 to 1:43", "Client review and sample record", "One named reviewer sees the promise, result, and evidence together, then makes a clear decision that prepares the invoicing handoff."],
            ["1:43 to 2:00", "Trust status board", "The public walkthrough is live and seeded. Account-based services require configuration. Broader production guarantees remain planned, not promised."],
          ],
        },
      },
      {
